...
from oaps4.libs.config import config as oaps4config
...
class LdapAdaptor(object):
    instances = {}
    def __new__ (cls, server=None, shared=True, options=None):
        #Default server
        if server is None:
            if options is None:
                options = Options()

            server = options.get_property("default_server","LdapAdaptor")

        #For web pages, we will need a new connection all the time
        if shared == False:
            if isinstance(server, cls._LdapAdaptor):
                return server.copy()
            else:
                return cls._LdapAdaptor(server, options)

        #In cases where we already have a _LdapAdaptor, juste return it.
        if isinstance(server, cls._LdapAdaptor):
            return server

        if server in cls.instances:
            return cls.instances[server]
        else:
            return cls.instances.setdefault(server, cls._LdapAdaptor(server, options))

    class _LdapAdaptor(object):
        """
        This class defines the adaptor to access an LDAP server an offers
        basic methods to manipulate objects in the server.
        """
        CASE_INSENSITIVE_DN = "case_insensitive_dn"
        def __init__ (self, section, config=None):
            """Creates the instance, initializing a connection and binding to the LDAP server."""
            self._section = section
            self._cfg = config or Options()

            self._connected = False
            self._bound = False
            self._ldap = None
            self._server_url = self.get_option("server_uri")
            self._binduser, self._bindpw = self.get_option("bind_user"), self.get_option("bind_password")
            self._base_dn = self.get_option("base_dn")
            self._cert_validation = self.has_option('certificate_validation') \
                                    and bool(int(self.get_option('certificate_validation')))

            # FIXME : Defer initialization until connection is needed
            # connect for each operation, then keep a cache of active connection
            #print self._server_url, self._binduser, self._bindpw
            self.initialize (self._server_url)
            self.bind(self._binduser, self._bindpw)

            # FIXME : p_version is a constant as coded like this
        def __del__(self):
            if self._ldap:
                self._ldap.unbind()

        def __str__(self):
            return "<LdapAdaptor: {0}>".format(self._section)
        ...
        def copy(self):
            return type(self)(self._section, self._cfg)

        def get_option (self, key, ptype=""):
            return self._cfg.get_property(key, self._section, ptype)

        def has_option (self, key):
            return self._cfg.has_property(key, self._section)

        def is_dry_run(self):
            return oaps4config.is_dry_run
        ...
        def initialize (self, server, p_version=ldap.VERSION3):
            ...
            try:
                ...
                if self.has_option("use_referrals"):
                    ldap.set_option(ldap.OPT_REFERRALS, self.get_option("use_referrals", "int"))
                else:
        ...
        def is_case_insensitive(self):
            return self.has_option(self.CASE_INSENSITIVE_DN) and \
                bool(int(self.get_option(self.CASE_INSENSITIVE_DN)))
